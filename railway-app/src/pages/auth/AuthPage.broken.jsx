...
// Validation and other logic below...
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signin') {
        // Sign in (user or employee)
        let result;
        if (loginType === 'employee') {
          // Employee login: call backend directly
          try {
            const res = await api.post('/session/employee/login', {
              email: form.email.trim().toLowerCase(),
              password: form.password,
            });
            result = res.data;
            // Normalize for toast and navigation
            if (result.status === 'success' && result.data && result.data.employee) {
              result.success = true;
              result.user = result.data.employee;
            } else {
              result.success = false;
              result.error = result.message || 'Login failed';
            }
          } catch (err) {
            result = { success: false, error: err.response?.data?.message || err.message || 'Login failed' };
          }
        } else {
          // User login (default)
          result = await login({
        ...
        if (result.success) {
          toast.success(`Welcome back, ${result.user.fullName || result.user.email || result.user.employeeId || ''}!`);
          // Redirect admin/employee to admin dashboard if role is admin
          ...
          if (result.error?.toLowerCase().includes('not registered')) {
