...
def generate_session_id() -> str:
    ...
    # 63 bits maps to 0..(2^63 - 1), which fits signed BIGINT max.
    session_int = secrets.randbits(63)
    return str(session_int)


def extract_raw_session_id(session_id_or_signed: str) -> Optional[str]:
    """
    Normalize a session identifier to raw DB Session_ID format.

    Accepts either:
      - Signed cookie value (value.signature)
      - Raw session ID (legacy/header fallback)

    Returns:
        Raw session ID string if valid, None otherwise.
    """
    if not session_id_or_signed:
        return None

    candidate = str(session_id_or_signed).strip()
    if not candidate:
        return None

    # Signed cookie path (primary flow)
    if "." in candidate:
        return unsign_cookie(candidate)

    # Backward-compatible raw session ID path
    return candidate
...
def create_session(
    ...
    user_type: str = 'user'  # 'user' for passengers, 'employee' for staff
) -> Tuple[str, str]:
    """
    Create a new session for a user or employee.
    
    Args:
        user_id: ROWID from Users table (if user_type='user') or Employees table (if user_type='employee')
        user_email: User's/Employee's email address
        user_role: Role ('Passenger' for users, 'Employee'/'Admin' for employees)
        ip_address: Client IP address
        user_agent: Client User-Agent header
        device_fingerprint: Optional device fingerprint data
        user_type: 'user' (passenger from Users table) or 'employee' (staff from Employees table)
    ...
    # User_ID references either Users.ROWID or Employees.ROWID based on User_Type
    session_data = {
    ...
    try:
        ...
        logger.info(f"Creating session for {user_type} {user_id}: {session_id[:8]}... with data keys: {list(session_data.keys())}")
        result = cloudscale_repo.create_record(TABLES['sessions'], session_data)
        
        
        if not result.get('success'):
            raise DatabaseError(f"Failed to create session: {result.get('error')}")
...
def validate_session(signed_session_id: str) -> Optional[Dict[str, Any]]:
    ...
    # Normalize to raw Session_ID used in DB queries
    session_id = extract_raw_session_id(signed_session_id)
    ...
    try:
        # Query session from CloudScale
        # Try to get User_Type if column exists, otherwise default to 'user'
        # Try with User_Type column first using CriteriaBuilder
        try:
            criteria = CriteriaBuilder() \
                .id_eq("Session_ID", session_id) \
                .eq("Is_Active", "true") \
                .build()
                
            query = f"""
                SELECT ROWID, Session_ID, User_ID, User_Type, User_Email, User_Role, IP_Address,
            ...
            # If User_Type column doesn't exist yet, try without it
            if 'User_Type' in str(e) or 'column' in str(e).lower():
                logger.warning("User_Type column not found in Sessions table, using fallback query")
                # Use CriteriaBuilder for fallback query
                criteria = CriteriaBuilder() \
                    .id_eq("Session_ID", session_id) \
                    .eq("Is_Active", "true") \
                    .build()
        ...
        return {
            ...
            "user_type": session.get("User_Type", "user"),  # 'user' or 'employee'
...
def revoke_session(session_id: str, user_id: Optional[str] = None) -> bool:
    ...
    """
    raw_session_id = extract_raw_session_id(session_id)
    if not raw_session_id:
        return False
    return _revoke_session_internal(raw_session_id, user_id=user_id, reason="user_logout")
...
def _revoke_session_internal(
    ...
    try:
        ...
        criteria = CriteriaBuilder().id_eq("Session_ID", session_id)
        if user_id:
...
def _update_session_activity(session_id: str, timestamp: datetime) -> None:
    ...
    try:
        # Get ROWID first using CriteriaBuilder
...
def validate_csrf_token(session_id: str, csrf_token: str) -> bool:
    ...
    try:
        ...
        criteria = CriteriaBuilder() \
            .id_eq("Session_ID", raw_session_id) \
            .eq("Is_Active", "true") \
            .build()
...
def regenerate_csrf_token(session_id: str) -> Optional[str]:
    ...
    try:
        ...
        # Get ROWID first using CriteriaBuilder 
        criteria = CriteriaBuilder().id_eq("Session_ID", raw_session_id).build()
        query = f"""
            SELECT ROWID FROM {TABLES['sessions']}
            WHERE {criteria}
        """
...
def _log_session_event(
    ...
    try:
        ...
        # Build audit data - only include User_ID if valid bigint
        audit_data = {
        ...
        # Only add User_ID if it's a valid positive integer ROWID (foreign key)
        # User_ID is a foreign key to Users/Employees table - must be valid ROWID or omitted
        if user_id and str(user_id).strip():
            try:
                user_id_int = int(user_id)
                # Foreign keys require valid ROWID (positive integer > 0)
                if user_id_int > 0:
                    audit_data["User_ID"] = user_id_int
                else:
                    logger.debug(f"Audit log: Skipping User_ID={user_id_int} (must be positive)")
            except (ValueError, TypeError) as e:
                # Skip User_ID if not a valid integer
                logger.debug(f"Audit log: Skipping invalid User_ID='{user_id}' type={type(user_id).__name__}")
                pass
        
        logger.debug(f"Creating audit log with data: {audit_data}")
