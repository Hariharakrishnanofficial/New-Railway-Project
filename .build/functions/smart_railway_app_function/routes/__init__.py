...
def register_blueprints(app):
    ...
    # Session-based Auth routes (new standard session ID system)
    from routes.session_auth import session_auth_bp
    app.register_blueprint(session_auth_bp)

    # Session Migration routes (admin only)
    from routes.session_migration import session_migration_bp
    app.register_blueprint(session_migration_bp)

    # Error Table Migration routes (admin only)
    from routes.error_migration import error_migration_bp
    app.register_blueprint(error_migration_bp)
    ...
    # RBAC introspection routes (admin UI support)
    ...
    # Development-only table tools (NEVER register in production)
    import os
    if os.getenv('APP_ENVIRONMENT') == 'development':
        from routes.dev_tables import dev_tables_bp
        app.register_blueprint(dev_tables_bp)
