...
def api_request(method, endpoint, data=None, headers=None, expected_codes=[200, 201]):
    ...
    try:
        ...
        print(f"❌ {method:6} {endpoint:35} | ERR | 0ms | {str(e)[:50]}")
...
def test_crud_module(module_name, table_endpoint, sample_data, auth_required=False):
    ...
    print(f"\\n📊 {module_name} Summary: {passed_operations}/{total_operations} operations passed")
...
if login_result['passed'] and login_result.get('response', {}).get('data', {}).get('token'):
    ...
    print(f"✅ Login successful, token updated: {results['auth_tokens']['user_token'][:30]}...")

# STEP 2: Test All Modules with CRUD Operations
print("\\n🏗️  STEP 2: COMPREHENSIVE CRUD TESTING")
print("-"*60)
...
# 9. BOOKINGS
booking_data = {
    'userId': results['auth_tokens'].get('user_token', '1'),  # Current user
    'trainId': '1',
    'journeyDate': SampleDataGenerator.future_date(),
    'sourceStationId': '1',
    'destinationStationId': '2',
...
for module_name, module_data in results['modules'].items():
    if 'crud_summary' in module_data:
        ...
        c_icon = "✅" if crud['create'] else "❌"
        r_icon = "✅" if crud['read'] else "❌"
        u_icon = "✅" if crud['update'] else "❌" if module_name != 'Admin_Logs' else "➖"
        d_icon = "✅" if crud['delete'] else "❌" if module_name != 'Admin_Logs' else "➖"
...
print(f"\\n💾 Detailed results saved to: tests/all_modules_crud_results.json")
