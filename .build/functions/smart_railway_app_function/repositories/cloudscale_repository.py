...
# Cached stable CA bundle location (avoids .build churn during `catalyst serve` reloads)
_STABLE_CA_BUNDLE_PATH: Optional[str] = None


def _get_stable_ca_bundle_path() -> Optional[str]:
    """Return a CA bundle path that remains valid across `.build` cleanups."""
    global _STABLE_CA_BUNDLE_PATH

    if _STABLE_CA_BUNDLE_PATH and os.path.exists(_STABLE_CA_BUNDLE_PATH):
        return _STABLE_CA_BUNDLE_PATH
    ...
    try:
        ...
        if os.path.exists(stable_path):
            _STABLE_CA_BUNDLE_PATH = stable_path
            return _STABLE_CA_BUNDLE_PATH
    except Exception:
        pass
    ...
    # If certifi bundle is already outside `.build`, it's stable enough.
    if '.build' not in str(candidate).lower():
        _STABLE_CA_BUNDLE_PATH = candidate
        return _STABLE_CA_BUNDLE_PATH
...
def _sanitize_tls_bundle_env() -> None:
    """Ensure TLS bundle env vars point to an existing/stable CA bundle."""
    ...
    for env_key in ('SSL_CERT_FILE', 'REQUESTS_CA_BUNDLE', 'CURL_CA_BUNDLE'):
        ...
        bundle_exists = os.path.exists(bundle_path_str)
        bundle_in_build = '.build' in bundle_path_str.lower()

        # Replace missing paths, or `.build` paths when we have a stable alternative.
        if (not bundle_exists) or (bundle_in_build and stable_bundle and stable_bundle != bundle_path_str):
            if stable_bundle:
                logger.warning(
                    "Replacing TLS bundle path in %s: %s -> %s",
                    env_key,
                    bundle_path_str,
                    stable_bundle,
                )
                os.environ[env_key] = stable_bundle
            else:
                logger.warning("Removing invalid TLS bundle path from %s: %s", env_key, bundle_path_str)
                os.environ.pop(env_key, None)

    # Best-effort: set a stable bundle for downstream libraries if available.
    if stable_bundle:
        for env_key in ('SSL_CERT_FILE', 'REQUESTS_CA_BUNDLE', 'CURL_CA_BUNDLE'):
            if os.environ.get(env_key) != stable_bundle:
                os.environ[env_key] = stable_bundle
...
class CloudScaleRepository:
    ...
    def _get_zcql(self):
        ...
        if not app:
            # Last-resort fallback (prefer init_catalyst() from handler request context).
            try:
                import zcatalyst_sdk
                logger.warning('Catalyst app not initialized; falling back to zcatalyst_sdk.initialize()')
                app = zcatalyst_sdk.initialize()
            except ImportError:
                raise RuntimeError("Catalyst app not initialized. Call init_catalyst() first.")
            except Exception as exc:
                raise RuntimeError(f"Catalyst app not initialized. Call init_catalyst() first. ({exc})")
        return app.zcql()
    ...
    def get_all_records(
        ...
        try:
            ...
            if actual_table == 'Stations':
                ...
                # Include KYC/verification fields used by admin UI.
                query = (
                    f"SELECT ROWID, Full_Name, Email, Password, Phone_Number, Role, "
                    f"Account_Status, Date_of_Birth, Address, Last_Login, "
                    f"Aadhar_Verified, ID_Proof_Type, ID_Proof_Number, Is_Verified, CREATEDTIME, MODIFIEDTIME "
                    f"FROM {actual_table}"
                )
                ...
                # Employees list/detail queries should NOT return Password hashes.
                # Schema varies across deployments, so we try a preferred field set and fall back.
                selects = [
                    "ROWID, Employee_ID, Full_Name, Email, Role, Department, Designation, Account_Status, Phone_Number, Joined_At, Is_Active, Last_Login",
                    "ROWID, Employee_ID, Full_Name, Email, Role, Account_Status, Phone_Number",
                    "ROWID, Full_Name, Email, Role, Account_Status",
                ]

                last_error = None
                for sel in selects:
                    query = f"SELECT {sel} FROM {actual_table}"
                ...
                return {"success": False, "error": last_error or "Failed to fetch Employees"}
            else:
    ...
    def get_record_by_id(self, table_name: str, record_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if actual_table == 'Stations':
                ...
                # Do not return Password hashes from generic record fetches.
                # Schema varies across deployments, so try a preferred field set and fall back.
                selects = [
                    "ROWID, Employee_ID, Full_Name, Email, Role, Department, Designation, Account_Status, Phone_Number, Joined_At, Is_Active, Last_Login",
                    "ROWID, Employee_ID, Full_Name, Email, Role, Account_Status, Phone_Number",
                    "ROWID, Full_Name, Email, Role, Account_Status",
                ...
                last_error = None
                for sel in selects:
                    query = f"SELECT {sel} FROM {actual_table} WHERE ROWID = {record_id}"
                    result = self.execute_query(query)
                    if result.get('success'):
                ...
                return {"success": False, "error": last_error or "Failed to fetch Employee"}
            else:
    ...
    def get_employee_by_email(self, email: str) -> Optional[Dict]:
        ...
        NOTE: This method is used for authentication, so it must include the Password hash.
        Schema varies across deployments, so we try a conservative select first and fall back.
        """
        ...
        actual_table = self._resolve_table(TABLES['employees'])

        selects = [
            # Preferred: includes common staff fields
            "ROWID, Employee_ID, Full_Name, Email, Password, Role, Department, Designation, Account_Status, Phone_Number, Joined_At, Is_Active, Last_Login",
            # Fallback: minimal auth + role
            "ROWID, Full_Name, Email, Password, Role, Account_Status",
        ]

        last_error = None
        for sel in selects:
            query = f"SELECT {sel} FROM {actual_table} WHERE {criteria} ORDER BY ROWID DESC LIMIT 1"
            result = self.execute_query(query)
            if result.get('success'):
                rows = result.get('data', {}).get('data', []) or []
                logger.info(f"[REPO] Query returned {len(rows)} records")
                if rows:
                    logger.info(f"[REPO] Found employee email: {rows[0].get('Email')}")
                    return rows[0]
                return None
            last_error = result.get('error')

        logger.info(f"[REPO] Employee lookup failed: {last_error}")
        return None

    def get_employee_details_by_id(self, employee_row_id: str) -> Optional[Dict]:
        """Fetch an employee by ROWID with a broad field set (excluding Password).

        Used for admin "details view". Schema varies across deployments, so we use
        fallback SELECT lists.
        """
        try:
            rid = int(str(employee_row_id).strip())
        except Exception:
            return None

        actual_table = self._resolve_table(TABLES['employees'])
        criteria = CriteriaBuilder().id_eq("ROWID", rid).build()

        selects = [
            # Preferred: broad details (no Password)
            "ROWID, CREATORID, CREATEDTIME, MODIFIEDTIME, Employee_ID, Full_Name, Email, Role, Department, Designation, Phone_Number, Account_Status, Is_Active, Joined_At, Last_Login, Address, Date_of_Birth, Aadhar_Verified, ID_Proof_Type, ID_Proof_Number, Permissions, Invitation_Id, Invited_By, User_ID",
            # Fallbacks: progressively smaller
            "ROWID, CREATEDTIME, MODIFIEDTIME, Employee_ID, Full_Name, Email, Role, Department, Designation, Phone_Number, Account_Status, Is_Active, Joined_At, Last_Login, Address",
            "ROWID, Employee_ID, Full_Name, Email, Role, Account_Status, Phone_Number, Department, Designation, Address",
            "ROWID, Full_Name, Email, Role, Account_Status",
        ]
        ...
        for sel in selects:
            ...
            if result.get('success'):
                ...
                if rows:
                    row = rows[0]
                    # Defense-in-depth: never return Password from details.
                    if isinstance(row, dict):
    ...
    def update_employee(self, employee_row_id: str, update_data: Dict) -> Dict[str, Any]:
        ...
        NOTE: Employees schema varies across deployments. If an update fails due to an
        invalid column, retry by dropping optional fields.
        """
        # Ensure email is lowercase if being updated
        if 'Email' in update_data:
            update_data['Email'] = str(update_data['Email']).lower()

        def _do_update(payload: Dict) -> Dict[str, Any]:
            return self.update_record(TABLES['employees'], employee_row_id, dict(payload))

        result = _do_update(update_data)
        if result.get("success"):
            self.invalidate_employee_cache(employee_row_id)
            return result

        err_lower = str(result.get('error') or '').lower()
        schema_markers = (
            'invalid column',
            'unknown column',
            'invalid column name',
        ...
        if not any(m in err_lower for m in schema_markers):
            return result

        # Drop schema-variant/optional fields and retry progressively.
        optional_fields = [
            'Department',
            'Designation',
            'Address',
