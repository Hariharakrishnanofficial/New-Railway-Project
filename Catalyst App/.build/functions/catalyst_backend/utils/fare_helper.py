...
def get_fare_for_journey(train_id, from_station_id, to_station_id, cls,
    ...
    try:
        ...
        records = zoho_repo.get_records(forms['reports']['fares'], criteria=criteria, limit=1)
    ...
    if str(quota or '').lower().strip() in ('tatkal', 'premium tatkal', 'tq', 'pt'):
        ...
        if tatkal_fare_from_record > 0:
            ...
            surcharge_pct = _get_tatkal_surcharge(quota, forms)
            if surcharge_pct > 0:
...
def _get_tatkal_surcharge(quota, forms) -> float:
    ...
    try:
        ...
        quotas = zoho_repo.get_records(forms['reports']['quotas'], criteria=criteria, limit=1)
        if quotas:
