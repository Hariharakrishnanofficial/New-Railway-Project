...
def get_quotas():
    result = zoho.get_all_records(_get_reports()['quotas'])
...
def get_quota(id):
    result = zoho.get_record_by_id(_get_reports()['quotas'], id)
    if not result.get('success'):
...
def create_quota():
    ...
    result = zoho.create_record(get_form_config()['forms']['quotas'], payload)
...
def update_quota(id):
    ...
    result = zoho.update_record(get_form_config()['reports']['quotas'], id, payload)
    if result.get('success'):
        user_email = request.headers.get('X-User-Email', 'Unknown')
...
def delete_quota(id):
    ...
    result = zoho.delete_record(get_form_config()['reports']['quotas'], id)
    if not result.get('success'):
...
def _get_forms():
    return get_form_config()['forms']

def _get_reports():
    return get_form_config()['reports']
