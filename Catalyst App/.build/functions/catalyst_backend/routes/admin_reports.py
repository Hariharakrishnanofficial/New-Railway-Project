...
def revenue_report():
    ...
    result = zoho.get_all_records(zoho.forms['reports']['bookings'], criteria=None, limit=1000)
...
def occupancy_report():
    ...
    # Fetch all trains
    trains_res = zoho.get_all_records(zoho.forms['reports']['trains'], criteria=None, limit=500)
    trains = trains_res.get('data', {}).get('data', []) if trains_res.get('success') else []

    # Fetch all bookings
    bookings_res = zoho.get_all_records(zoho.forms['reports']['bookings'], criteria=None, limit=1000)
...
def get_admin_logs():
    ...
    result = zoho.get_all_records(zoho.forms['reports']['admin_logs'], criteria=criteria, limit=limit)
...
def create_admin_log():
    ...
    result = zoho.create_record(zoho.forms['forms']['admin_logs'], payload)
    return jsonify(result), result.get('status_code', 200)
