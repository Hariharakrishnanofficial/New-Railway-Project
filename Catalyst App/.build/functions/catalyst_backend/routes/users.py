...
def get_users():
    ...
    result = zoho.get_all_records(zoho.forms['reports']['users'], cb.build(), limit)
...
def get_user(user_id):
    ...
    result = zoho.get_record_by_id(zoho.forms['reports']['users'], user_id)
    return jsonify(result), result.get('status_code', 200)
...
def update_user(user_id):
    ...
    result = zoho.update_record(zoho.forms['reports']['users'], user_id, payload)
...
def delete_user(user_id):
    ...
    result = zoho.delete_record(zoho.forms['reports']['users'], user_id)
    if result.get('success'):
        zoho_repo.invalidate_user_cache(user_id)
    return jsonify(result), result.get('status_code', 200)
...
def get_user_bookings(user_id):
    ...
    result = zoho.get_all_records(zoho.forms['reports']['bookings'], criteria=criteria, limit=200)
...
def update_account_status(user_id):
    ...
    result = zoho.update_record(zoho.forms['reports']['users'], user_id, {'Account_Status': new_status})
    if result.get('success'):
        zoho_repo.invalidate_user_cache(user_id)
...
def get_user_insights(user_id):
    ...
    bookings = zoho_repo.get_records(zoho.forms['reports']['bookings'], criteria=cb, limit=100)
