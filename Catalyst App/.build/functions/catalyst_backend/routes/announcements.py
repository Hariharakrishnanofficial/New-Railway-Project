...
def get_announcements():
    ...
    result = zoho.get_all_records(zoho.forms['reports']['announcements'], criteria=None, limit=limit)
...
def get_active_announcements():
    ...
    result = zoho.get_all_records(zoho.forms['reports']['announcements'], criteria=None, limit=200)
    if not result.get('success'):
        return jsonify(result), result.get('status_code', 500)
...
def get_announcement(ann_id):
    result = zoho.get_record_by_id(zoho.forms['reports']['announcements'], ann_id)
...
def create_announcement():
    ...
    result = zoho.create_record(zoho.forms['forms']['announcements'], payload)
    return jsonify(result), result.get('status_code', 200)
...
def update_announcement(ann_id):
    ...
    result = zoho.update_record(zoho.forms['reports']['announcements'], ann_id, payload)
    return jsonify(result), result.get('status_code', 200)
...
def delete_announcement(ann_id):
    result = zoho.delete_record(zoho.forms['reports']['announcements'], ann_id)
    return jsonify(result), result.get('status_code', 200)
