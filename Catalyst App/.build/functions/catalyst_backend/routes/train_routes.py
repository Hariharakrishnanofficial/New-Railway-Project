...
def _fetch_all_routes_full(limit=500):
    ...
    stops_res = zoho.get_all_records(
        zoho.forms['reports']['route_stops'],
        criteria=None,
        limit=2000
    )
    all_stops = []
...
def add_route_stop(route_id):
    ...
    result = zoho.create_record(zoho.forms['forms']['route_stops'], stop_payload)
    if result.get('success'):
        record_id = result.get('data', {}).get('ID')
...
def update_route_stop(route_id, stop_id):
    ...
    result = zoho.update_record(zoho.forms['reports']['route_stops'], stop_id, stop_payload)
    if result.get('success'):
...
def delete_route_stop(route_id, stop_id):
    ...
    result = zoho.delete_record(zoho.forms['reports']['route_stops'], stop_id)
    if result.get('success'):
