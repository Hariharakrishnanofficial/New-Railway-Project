...
//   Uses Catalyst Advanced I/O Function endpoint (serverless).
//   Function name: catalyst_backend
const DEPLOYED_BACKEND = 'https://smart-railway-app-60066581545.development.catalystserverless.in/server/catalyst_backend/api';
...
client.interceptors.response.use(
  ...
  async (err) => {
    ...
    // BUG FIX: CloudScale responses can wrap messages inside nested objects.
    // Previously only checked .error and .message at the top level, so nested
...
/**
 * CloudScale response shape: { success: true, data: { code: 3000, data: [...] }, status_code: 200 }
 * Handles all common nesting patterns returned by backend adapters.
 */
...
/** Record ID is uppercase "ID" in current backend responses */
export function getRecordId(row) {
  return row?.ID ?? row?.id ?? null;
}

/** Lookup fields are often { ID, display_value } — extract label for display */
...
/** "YYYY-MM-DDTHH:MM" → "DD-MMM-YYYY HH:MM:SS"  (backend datetime format) */
...
// ─── CloudScale-friendly aliases (keep Zoho names for backward compatibility) ─
export const parseCloudDate = parseZohoDate;
export const parseCloudDateOnly = parseZohoDateOnly;
export const displayCloudDate = displayZohoDate;
export const toCloudDateTime = toZohoDateTime;
export const displayCloudDateTime = displayZohoDateTime;
...
//
// DATABASE DESIGN (CloudScale-compatible):
...
export const aiApi = {
  // Natural language → CloudScale-backed search results
  search: (query) =>
  ...
  // MCP structured query execution against CloudScale tables
  mcpQuery: (payload) =>
