...
from config import (
    COACH_PREFIX, COACH_CAPACITY, BERTH_CYCLE, TABLES,
...
def get_train_inventory(train_id, journey_date, cls):
    ...
    inv_report = TABLES['train_inventory']
    ...
    inv_form = TABLES['train_inventory']
...
def process_booking_allocation(train_id, cls, journey_date, passengers):
    ...
    zoho.update_record(TABLES['train_inventory'], inv['ID'], update_payload)
...
def promote_waitlist(train_id, cls, journey_date, num_seats_to_fill):
    ...
    for booking in promotable_bookings:
        ...
        if num_to_promote <= num_seats_to_fill:
            ...
            zoho.update_record(TABLES['bookings'], booking['ID'], update_payload)
