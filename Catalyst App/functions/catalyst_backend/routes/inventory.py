...
def get_inventory():
    ...
    result = zoho.get_all_records(TABLES['train_inventory'], criteria=criteria)
...
def get_inventory_by_id(id):
    result = zoho.get_record_by_id(TABLES['train_inventory'], id)
    if not result.get('success'):
...
def create_inventory():
    ...
    result = zoho.create_record(TABLES['train_inventory'], payload)
    if not result.get('success'):
...
def update_inventory(id):
    ...
    result = zoho.update_record(TABLES['train_inventory'], id, payload)
    if not result.get('success'):
...
def delete_inventory(id):
    result = zoho.delete_record(TABLES['train_inventory'], id)
    if not result.get('success'):
...
def prepare_chart():
    ...
    result = zoho.get_all_records(TABLES['train_inventory'], criteria=criteria, limit=20)
    ...
    updated = 0
    for r in records:
        upd = zoho.update_record(TABLES['train_inventory'], r['ID'], {'Chart_Prepared': True})
        if upd.get('success'):
            updated += 1
