...
def forgot_password():
    ...
    records = cloudscale_repo.get_records(TABLES['users'],
    ...
    cloudscale_repo.create_record(TABLES['reset_tokens'], {
...
def reset_password():
    ...
    tokens = cloudscale_repo.get_records(TABLES['reset_tokens'],
    ...
    users = cloudscale_repo.get_records(TABLES['users'],
                              criteria=f'(Email == "{email}")', limit=1)
    if not users:
        return jsonify({'success': False, 'error': 'User not found'}), 404

    cloudscale_repo.update_record(TABLES['users'], users[0]['ID'],
                       {'Password': hash_password(new_password)})
    cloudscale_repo.update_record(TABLES['reset_tokens'], valid['ID'], {'Is_Used': 'true'})
...
def setup_admin():
    ...
    from config import ADMIN_EMAIL, ADMIN_DOMAIN
    from core.security import hash_password
    ...
    existing = cloudscale_repo.get_records(TABLES['users'],
                                 criteria=f'(Email == "{target_email}")', limit=1)
    hashed   = hash_password(password)

    if existing:
        cloudscale_repo.update_record(TABLES['users'], existing[0]['ID'],
    ...
    cloudscale_repo.create_record(TABLES['users'], {
