...
def create_station():
    ...
    result = zoho.create_record(TABLES['stations'], payload)
...
def get_station(station_id):
    result = zoho.get_record_by_id(TABLES['stations'], station_id)
    return jsonify(result), result.get('status_code', 200)
...
def update_station(station_id):
    ...
    result = zoho.update_record(TABLES['stations'], station_id, payload)
    if result.get('success'):
...
def delete_station(station_id):
    result = zoho.delete_record(TABLES['stations'], station_id)
    if result.get('success'):
        cache.delete('stations:all')
    return jsonify(result), result.get('status_code', 200)
...
def bulk_create_stations():
    ...
    for i, s in enumerate(data['stations']):
        ...
        res = zoho.create_record(TABLES['stations'], payload)
...
def station_manifest(station_id):
    ...
    station_res = zoho.get_record_by_id(TABLES['stations'], station_id)
    ...
    bookings = zoho_repo.get_records(TABLES['bookings'], criteria=cb.build(), limit=500)
