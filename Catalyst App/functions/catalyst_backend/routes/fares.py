...
def _concessions_table():
    """
    Return configured concessions table name if present.
    CloudScale v2 schema does not define this table by default.
    """
    return TABLES.get('concessions')
...
def get_fares():
    try:
        ...
        data = zoho.get_all_records(TABLES['fares'], criteria=criteria)
...
def get_fare(id):
    try:
        data = zoho.get_record_by_id(TABLES['fares'], id)
        return jsonify(data), data.get('status_code', 200)
    except Exception as e:
        return jsonify({"success": False, "error": str(e), "status_code": 500}), 500
...
def create_fare():
    try:
        ...
        result = zoho.create_record(TABLES['fares'], payload)
...
def update_fare(id):
    try:
        ...
        result = zoho.update_record(TABLES['fares'], id, payload)
...
def delete_fare(id):
    try:
        result = zoho.delete_record(TABLES['fares'], id)
        return jsonify(result), result.get('status_code', 200)
    except Exception as e:
        return jsonify({"success": False, "error": str(e), "status_code": 500}), 500
...
def calculate_fare():
    ...
    try:
        ...
        train_result = zoho.get_record_by_id(TABLES['trains'], train_id)
        ...
        fares_result = zoho.get_all_records(TABLES['fares'], criteria=criteria)
...
def get_concessions():
    try:
        concessions_table = _concessions_table()
        if not concessions_table:
            return jsonify({
                "success": False,
                "error": "Concessions table is not configured in CloudScale TABLES",
                "status_code": 501
            }), 501

        params = request.args
        criteria = 'Is_Active == true' if params.get('is_active') == 'true' else ""
        data = zoho.get_all_records(concessions_table, criteria=criteria)
...
def create_concession():
    try:
        ...
        data = request.get_json()
        result = zoho.create_record(concessions_table, data)
...
def update_concession(id):
    try:
        ...
        data = request.get_json()
        result = zoho.update_record(concessions_table, id, data)
...
def delete_concession(id):
    try:
        ...
        result = zoho.delete_record(concessions_table, id)
        return jsonify({"success": True, "data": result, "status_code": 200})
    except Exception as e:
        return jsonify({"success": False, "error": str(e), "status_code": 500}), 500
